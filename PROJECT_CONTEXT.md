...
| First response | Handoff → first **human** agent message (`first_agent_message_at`, from Messaging tag flips; the bot never flips). Credited to `first_response_agent_id` = the assignee at that moment. **Live dashboard/TV** (`/api/wa-dashboard`): counts by the day the reply actually happened, not the day the ticket opened — a ticket that arrived after hours yesterday and got its first reply at 08:05 today is today's 5-minute first response (`WaExtraActivity.respondedToday`, account owner's rule 2026-09-14, ticket picker showing only 3/3 agents). **Not yet applied to `recompute_wa_agent_daily`** (the stored record behind "ביצועי WA") — that still credits by the ticket's opening day only, a known gap between the two screens |
| Tiers | < 3 min (green) / ≥ 3 / ≥ 7 / ≥ 10 (nested); unanswered open tickets count live |
| Time to close | Opening → `solved_at` (exact solved transition; `zendesk_updated_at` fallback). Credited to `solved_by_agent_id`. Same live-dashboard-only "day it happened" rule as first response above (`WaExtraActivity.closedToday`) |
