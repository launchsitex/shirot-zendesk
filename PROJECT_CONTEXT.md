...
| First response, split (דשבורד WA + TV WA only, 2026-09-14) | **"זמן תגובה מוקד"** = the figure above, unchanged (handoff → first message, includes any time unassigned in the queue). **"זמן תגובה נציגה"** = `assigned_to_agent_at` → first message — from the moment a person was actually assigned, not the bot's handoff to the queue. Ticket 74539: handoff 11:18, actually assigned to the agent only at 12:28 (70 min unassigned), her reply at 12:35 — "מוקד" 76 min, "נציגה" 7 min. `assigned_to_agent_at` = the last real assignee transition (`zendesk_ticket_transitions`, excluding the bot's clear-to-'0') at/before `first_agent_message_at`; null before 2026-09-09 (when transitions started) or if answered before any real assignee transition. Shown **side by side, not merged** — the account owner wants both averages before deciding which feeds bonuses. **Not yet applied to `recompute_wa_agent_daily` / "ביצועי WA"** — only `assigned_to_agent_at` itself is computed there (`recompute_ticket_transitions`), not a stored average |
