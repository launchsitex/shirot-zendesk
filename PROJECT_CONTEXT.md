...
| Figure | Definition |
| --- | --- |
| Day | Israel calendar day the ticket was **opened**; `via_channel = whatsapp`, `status <> deleted`. **ticketCount only** — respondedCount/closedCount below follow their own day (2026-09-14) |
| Tickets / open / closed | ticketCount credited to the **current** assignee, tickets **opened** that day only; closed = `solved` or `closed` |
...
| First response | Handoff → first **human** agent message (`first_agent_message_at`, from Messaging tag flips; the bot never flips). Credited to `first_response_agent_id` = the assignee at that moment. Counts by the day the reply actually happened, not the day the ticket opened — a ticket that arrived after hours yesterday and got its first reply at 08:05 today is today's 5-minute first response (live dashboard: `WaExtraActivity.respondedToday`; stored record: `recompute_wa_agent_daily`'s `activity` branch). Account owner's rule, 2026-09-14 — ticket picker was showing only 3/3 agents. **Only from 2026-09-14 on** — days before that keep the old "credited to the ticket's opening day" behavior forever, on both screens, so already-reported historical figures never shift (`v_activity_day_cutoff` in the SQL function) |
| First response, split ("זמן תגובה מוקד" / "זמן תגובה נציגה", 2026-09-14, all three WA screens) | **"זמן תגובה מוקד"** = the figure above, unchanged (handoff → first message, includes any time unassigned in the queue). **"זמן תגובה נציגה"** = `assigned_to_agent_at` → first message — from the moment a person was actually assigned, not the bot's handoff to the queue. Ticket 74539: handoff 11:18, actually assigned to the agent only at 12:28 (70 min unassigned), her reply at 12:35 — "מוקד" 76 min, "נציגה" 7 min. `assigned_to_agent_at` = the last real assignee transition (`zendesk_ticket_transitions`, excluding the bot's clear-to-'0') at/before `first_agent_message_at`; null before 2026-09-09 (when transitions started) or if answered before any real assignee transition. Shown **side by side, not merged** — the account owner wants both averages before deciding which feeds bonuses. Live on דשבורד WA + TV WA (`agentResponseSeconds` on `WaTicketRow`) **and** stored in `wa_agent_daily` for "ביצועי WA" (`agent_response_seconds_sum` / `agent_responded_count`, populated by `recompute_wa_agent_daily` — purely additive columns, no cutoff-date branch needed since no existing figure changes) |
| Department transfers (2026-09-24) | "תגובה מוקד"'s clock start (`clockStart` in `mapTicketRow`) prefers `entered_department_at` over `handed_to_agent_at`/`zendesk_created_at` when set — the last time the ticket's Zendesk group changed to one mapped (`zendesk_group_departments`) to its **current** agent's department. A ticket created in one department and transferred into another later is measured from the transfer, not the original handoff — otherwise the receiving department is blamed for a wait it never owned (found via ticket #82582: created in אספקות 05:17, transferred to שירות לקוחות 11:50, answered 14:58 — 8 minutes after the transfer, not 6 hours after creation). Populated going forward only by `recompute_ticket_transitions` from `zendesk_ticket_transitions` rows of `kind = 'group'` (captured by the sync since 2026-09-24; historical tickets are not backfilled, account owner's explicit call). `transferred_from_department_name` carries the prior department for display ("הועברה מ-X ב-HH:MM" next to the ticket in "ממתינים לתגובה"/"ענינו, ממתינות ללקוח") |
| Tiers | < 3 min (green) / ≥ 3 / ≥ 7 / ≥ 10 (nested); unanswered open tickets count live |
| "מענה ממוצע לכלל ההודעות" (per-agent, "פירוט לפי נציגה" on דשבורד WA only, 2026-09-22) | Average seconds from **every** customer WhatsApp turn (not just the first, unlike the two figures above) to the agent's next reply, pooled from raw `zendesk_whatsapp_messages` rows for that agent's tickets opened today (`averageMessageResponseByAgent` in `src/lib/wa-dashboard.ts`, queried per-request in the API route — not stored/daily). Each row already represents a turn switch (`whatsappFlip` only fires on a tag *change*), so a burst of consecutive customer messages before any reply counts once, not once per WhatsApp bubble. `handoff` counts as a turn start too (same moment "תגובה מוקד" starts from). Credited to the ticket's **current** assignee for every pair in it, even if it changed hands mid-conversation — same attribution as `ticketCount`, not the split first-response/close attribution above. Shown with its pair count in parentheses; "—" with zero pairs yet |
| Time to close | Opening → `solved_at` (exact solved transition; `zendesk_updated_at` fallback). Credited to `solved_by_agent_id`. Same "day it happened, only from 2026-09-14" rule as first response above (`WaExtraActivity.closedToday` / the SQL function's `activity` branch) |
| Waiting for reply | **Status `open` on the *default* custom status only** (`custom_status_id` matches the "open" category's default — "פתוחה" — in `zendesk_custom_statuses`; other open sub-statuses like "תזכורת" or "תאום לקוח" mean the agent already parked it on purpose and don't count, 2026-09-10, ticket #69875). A ticket synced before `custom_status_id` existed is treated as default so it keeps showing. Customer wrote last: since `customer_waiting_since` (their first message after the agent's last), or since handoff when no agent has written. Covers today **plus** open tickets opened on/after **2026-09-08** (`BACKLOG_FROM_DATE`), the first day with complete tag-flip data. **Live wait time is wall-clock, not business hours** (`currentlyWaiting` in `src/lib/wa-dashboard.ts`, 2026-09-14) — the customer is still waiting outside business hours even though no agent is measured on that time; same reasoning as the queue's own wall clock. Only this live view and its escalation-tier colors (3/7/10 min) — the stored/historical averages stay on the business clock |
...
| Agent picker | Excludes an agent from **every** figure on the screen; per department, localStorage, shared by dashboard and TV (`src/lib/wa-agent-filter.ts`). Lists every agent in `byAgent` — today's ticketCount **or** a reply/close credited to her today, even on an older ticket |
| "ענינו, ממתינות ללקוח" (2026-09-14, display only) | Green list next to "ממתינים לתגובה" on דשבורד WA + TV WA: open tickets on Zendesk status **pending** (agent already answered the customer's last message, ball in their court) — `WaDashboardPayload.pendingReplies`. Each row's time is **not** a general response-time figure — it's specifically how long the agent took to answer that customer's *last* message (`lastCustomerMessageAt` → `lastAgentMessageAt`), null without both **and only when the agent's message is actually after the customer's** — `last_*_message_at` are each just the latest of their kind, not a matched pair, so a "pending" ticket can still show the customer's one out-pacing the agent's (a status-flip sync lag, e.g.); shows "—" then instead of a misleading ~0-second reply. Live-only (today), does **not** feed any average/calculation — display only, per account owner's explicit request |
...
| Agent roles | `agents.role` → `agent_roles` (managed in Settings → "תפקידי נציגות": add/rename/reorder/delete; `agent` is the default and undeletable). Set per agent by admins in "נציגים וצוותים" (`PATCH /api/agents`). **Only** the availability boxes read it — grouped in the Settings order, answering agents (`agent`) first, headline counts on answering agents only. Never used for crediting tickets or pay. `src/lib/agent-roles.ts`, `/api/settings/agent-roles` |

**Manual exclusion from averages (2026-09-22):** `zendesk_tickets.excluded_from_wa_averages` (+ `..._reason`) — a ticket the account owner has personally confirmed is a genuine anomaly (first use: 13 tickets never routed/offered to any agent, sitting fully unassigned 3-8 hours despite near-instant pickup on everything else that day) drops out of every response-time/close-time average and tier count (`WaTicketRow.excludedFromAverages`, filtered in `summarizeTickets`/`summarizeByAgent`/`firstResponseTierCounts`/`firstResponseUnderCount` in `src/lib/wa-dashboard.ts`, and out of the ticket set fed to `averageMessageResponseByAgent` in the API route) — but stays in `ticketCount`/`closedCount`/`awaitingReply`, since it is a real ticket. Deliberately **not** an automatic threshold rule — each case is reviewed and flagged by hand.
...
**WhatsApp template sends** (2026-09-22) don't go through that tag-flip mechanism at all — Zendesk instead leaves a private auto-generated Comment event whose body starts with the fixed prefix "נשלחה הודעת WhatsApp עם תבנית:" (template name varies, `via.channel: "api"`; that field alone isn't a reliable signal — Aircall's own call-log notes are also "api"). `sync-zendesk-tickets` (`isWhatsappTemplateSentNote`) matches on that body prefix and records it into `zendesk_whatsapp_messages` as a normal `direction: 'agent'` row, so a template send stops "ממתינים לתגובה" and counts toward "תגובה נציגה"/"תגובה מוקד" exactly like a chat reply. Before this fix a template send never registered — ticket #78940 sat "unanswered" 5+ days despite one having been sent.

**"מעל 10 דק'" email alert (2026-09-22):** `notify-wa-waiting` edge function, polled by pg_cron `wa-waiting-alert-every-5-minutes` (no single row-transition to hang a trigger off, unlike the missed-call alert below — a ticket becomes alert-worthy purely by sitting idle). Calls SQL function `public.find_wa_waiting_alerts(threshold_seconds)`, which mirrors `mapTicketRow`'s `waitingSince` gating exactly (status/custom-status/no-agent-reply-yet-cutoff) and measures elapsed with `public.business_seconds` — keep both in sync if that logic changes. Emails (Resend) the same recipients as the missed-call alert (`missed_call_notification_recipients`/`missed_call_notification_settings.from_email`) once per distinct wait (`zendesk_tickets.waiting_alert_sent_for` dedupes on the ticket's current `effective_waiting_since`; a new wait after an agent reply alerts again). Display/ops only — does not feed any figure or average.
...
| `sync-zendesk-tickets` | off | Every minute (pg_cron): incremental tickets + events export, WhatsApp tag flips, transitions, agent availability (v10). Also: `POST {"check_ticket":"<id>"}` — isolated read-only lookup (per-ticket Zendesk comments endpoint, not the incremental export) for whether a ticket has a WhatsApp-template private note; touches no cursor, cannot affect live sync (added 2026-09-22 for spot-checking stuck TV tickets without a history rewind). Also: `POST {"get_transcripts":["<id>",...]}` (up to 40) — same isolated per-ticket comments endpoint, returns the full transcript (every speaker, agent/customer/bot) for manual conversation-quality review; content is never stored in our DB (added 2026-09-22, v16) |
